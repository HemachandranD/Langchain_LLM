...
def lookup(source: str) -> str:
    ...
    template = """Give the source {name_of_source}, get the URL of the conytent.Your answer should contain only a URL."""
    # Instantiation using from_template (recommended)
    prompt = PromptTemplate.from_template(template)
    prompt.format(name_of_source=source)
    tools_for_agent = [
        Tool(
            name="Crawl Top most news headlines and get the URL of the article",
    ...
    result = agentexecutor.invoke(input={"input": source})
